...
export default function BattleArena({
  ...
  const castVote = async (battleId: string, team: 'A' | 'B') => {
    ...
    try {
      ...
      console.error('Failed to register tactical vote', e);
    }
  };
  ...
  const handlePostComment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Failed to post discussion take', e);
    }
  };

  const createBattle = async () => {
    if (!squadAId || !squadBId) {
      alert('Must select two valid hybrid squads to initiate combat!');
      return;
    }
    if (squadAId === squadBId) {
      alert('Cannot battle a squad against itself! That breaks match laws.');
      return;
    }
    ...
    try {
      ...
      console.error('Failed to challenge teams', e);
    } finally {
    ...
    <div className="w-full max-w-5xl mx-auto p-4 select-none">
      ...
      <div className="flex justify-between items-center mb-6 pb-2 border-b border-white/5 flex-wrap gap-4 select-none">
        <div>
          ...
          <h1 className="text-xl font-black text-white uppercase tracking-tight">SQUAD COMBAT ARENA</h1>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 font-mono text-xs">
        ...
        <div className="lg:col-span-4 flex flex-col gap-3 max-h-[500px] overflow-y-auto">
          <span className="text-[10px] text-gray-400 uppercase tracking-widest block font-bold mb-1">ACTIVE DUELS:</span>
          {battles.length === 0 ? (
            <div className="text-center py-10 bg-slate-900 border border-white/5 rounded-2xl text-gray-500">
              No active duels compiled. Launch a duel above!
            </div>
          ) : (
            battles.map(bat => {
                ...
                >
                  <div className="flex justify-between items-baseline mb-2">
                    <span className="text-[10px] text-emerald-400 font-bold uppercase tracking-wider flex items-center gap-1">
                      <Swords className="w-3 h-3 text-emerald-400" /> Duel
                    </span>
                  ...
                  <div className="text-yellow-400 font-extrabold text-[10px] italic mb-1">VS</div>
        ...
        <div className="lg:col-span-8 flex flex-col gap-5">
          {activeBattle ? (
            ...
            >
                    ...
                    <div className="flex flex-col gap-5">
                      ...
                      <div className="flex justify-between items-center sm:px-6 flex-col sm:flex-row gap-4 select-none">
                        ...
                        <div className="text-center sm:text-left flex-1 font-sans">
                          <h3 className="font-extrabold text-sm uppercase text-gray-500 font-mono tracking-widest leading-none mb-1">SQUAD A</h3>
                          <h2 className="text-base font-black text-white uppercase truncate">{activeBattle.squadA.name}</h2>
                          <div className="text-[11px] text-emerald-400 font-bold uppercase tracking-wide mt-1">OVR: {activeBattle.squadA.rating} • CHEM: {activeBattle.squadA.chemistry}%</div>
                          <button
                          ...
                          >
                            Vote {activeBattle.squadA.name}
                          </button>
                        </div>

                        <div className="text-center font-sans">
                          <div className="text-2xl font-black text-yellow-400 italic">VS</div>
                          <span className="text-[9px] font-mono text-gray-500 block mt-1 uppercase">ARENA POLL</span>
                        </div>

                        {/* Squad B */}
                        <div className="text-center sm:text-right flex-1 font-sans">
                          <h3 className="font-extrabold text-sm uppercase text-gray-500 font-mono tracking-widest leading-none mb-1">SQUAD B</h3>
                          <h2 className="text-base font-black text-white uppercase truncate">{activeBattle.squadB.name}</h2>
                          <div className="text-[11px] text-emerald-400 font-bold uppercase tracking-wide mt-1">OVR: {activeBattle.squadB.rating} • CHEM: {activeBattle.squadB.chemistry}%</div>
                          <button
                            onClick={() => castVote(activeBattle.id, 'B')}
                            className="mt-3 bg-emerald-900/40 border border-emerald-500/25 text-emerald-400 hover:text-white hover:bg-emerald-800 transition px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-wider cursor-pointer"
                          >
                            Vote {activeBattle.squadB.name}
                          </button>
                        </div>
                      </div>
                      ...
                      <div className="flex justify-between text-[10px] text-gray-400 font-bold select-none px-1">
                        <span>{activeBattle.votesA} TOTAL VOTES CAST</span>
                        <span>{activeBattle.votesB} VOTES CAST</span>
                      </div>
                    </div>
                  );
                })()}
              ...
              <div className="bg-slate-900/80 border border-white/5 p-5 rounded-2xl flex flex-col gap-4 shadow-xl">
                <span className="text-[10px] text-gray-400 font-black uppercase tracking-wider block border-b border-white/5 pb-2">
                  ...
                  TACTICAL DISCUSSION FEED ({activeComments.length})
                </span>
                ...
                <form onSubmit={handlePostComment} className="flex gap-2">
                  <input
                    ...
                    placeholder="Publish your squad analytical counter-theory or trash talk..."
                ...
                <div className="flex flex-col gap-3 max-h-[250px] overflow-y-auto">
                  {loadingComments ? (
                    <div className="text-center py-6 text-gray-500">Loading opinions...</div>
                  ) : activeComments.length === 0 ? (
                    <div className="text-center py-8 text-gray-500 flex flex-col items-center">
                      <Flame className="w-6 h-6 text-slate-700 mb-1" />
                      Debates are cool down. Post first take to ignite fire!
                    </div>
                  ) : (
            ...
            <div className="p-10 text-center bg-slate-900/60 border border-white/5 rounded-2xl text-gray-400 font-mono">
              Choose an active duel on the left to review telemetry debate statistics or cast votes!
            </div>
          )}
        </div>
          ...
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            ...
            >
              ...
              <div className="flex flex-col gap-5 font-mono text-xs">
                ...
                {/* Squad 1 selection */}
                <div>
                  <label className="text-[10px] text-gray-400 uppercase tracking-widest block mb-1">CHALLENGE SQUAD A:</label>
                  <select
                    value={squadAId}
                  ...
                  >
                    <option value="">-- PICK COMBATANT SQUAD --</option>
                    {battleEligibleSquads.map(sq => (
                      <option key={sq.id} value={sq.id}>
                        🛡️ {sq.name} (OVR: {sq.rating}, CHEM: {sq.chemistry}%) - @{sq.userName}
                      </option>
                    ))}
                  </select>
                </div>
                ...
                {/* Squad 2 selection */}
                <div>
                  <label className="text-[10px] text-gray-400 uppercase tracking-widest block mb-1 font-mono">CHALLENGE SQUAD B:</label>
                  <select
                    value={squadBId}
                ...
                <div className="bg-amber-500/5 p-3 rounded-xl border border-amber-500/10 flex gap-2.5 items-start mt-1">
                  ...
                  <p className="text-[10px] text-gray-400 leading-normal font-sans">
                    Locking this duel creates a public voting deck. Fans can review overall ratings, chemistry linkages, playstyle, and cast live votes!
                  </p>
                </div>
                ...
                <div className="flex justify-end gap-3 mt-4 pt-4 border-t border-white/5 font-mono">
                  ...
                  >
                    {submittingBattle ? 'LAUNCHING...' : 'LAUNCH DUEL'}
                  </button>
                </div>
